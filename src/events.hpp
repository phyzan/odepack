...
public:
    ...
    virtual bool determine(const Tt& t1, const Tt& t2, const std::vector<Tt>& args, const std::function<Ty(const Tt&)>& q, const Tt& tol) = 0;
...
public:
    ...
    bool determine(const Tt& t1, const Tt& t2, const std::vector<Tt>& args, const std::function<Ty(const Tt&)>& q, const Tt& tol) override {
        this->_clear();
        Tt t_determined = t2;
        bool determined = false;
        if (this->_check_if == nullptr || (this->_check_if(t1, q(t1), args) && this->_check_if(t2, q(t2), args))){
            _ObjFun<Tt> obj_fun = [this, q, args](const Tt& t) ->Tt {
                return this->_when(t, q(t), args);
            };
            Tt val1 = this->_when(t1, q(t1), args);
            Tt val2 = this->_when(t2, q(t2), args);
