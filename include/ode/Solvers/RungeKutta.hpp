...
public:
    ...
    RK4(MAIN_DEFAULT_CONSTRUCTOR(T)) requires (!is_rich<SP>): Base(ode, t0, q0, nsys, rtol, atol, 0, inf<T>(), first_step, dir, args), K(5, nsys){
        // min_step and max_step are not used in RK4
    }

    RK4(MAIN_DEFAULT_CONSTRUCTOR(T), EVENTS events = {}) requires (is_rich<SP>): Base(ode, t0, q0, nsys, rtol, atol, 0, inf<T>(), first_step, dir, args, events), K(5, nsys){
        // min_step and max_step are not used in RK4
    }
