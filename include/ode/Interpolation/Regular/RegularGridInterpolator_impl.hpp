...
template<typename T, int NDIM, bool AS_VIRTUAL>
std::vector<Array2D<T, NDIM, 0>> RegularVectorField<T, NDIM, AS_VIRTUAL>::streamplot_data(const T& max_length, const T& ds, size_t density) const{
...
template<typename T, int NDIM, bool AS_VIRTUAL>
template<size_t... I>
std::vector<Array2D<T, NDIM, 0>> RegularVectorField<T, NDIM, AS_VIRTUAL>::streamplot_data_core(const T& max_length, const T& ds, size_t density, std::index_sequence<I...>) const{
    ...
    Solver solver(OdeData<Func<T>, void>{.rhs=VFBase::ode_func_norm, .obj=this}, 0, nullptr, this->ndim(), 1e-5, 1e-5, 0, inf<T>(), ds, +1, static_cast<const std::vector<T>&>(args), static_cast<const std::vector<const Event<T>*>&>(events));
    ...
    auto IntegrateDirection = [&](Solver& solver, T& s_total, T* const * x, int& n_steps_tot, const int dir) -> int {
