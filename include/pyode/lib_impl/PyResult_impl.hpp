...
const OdeResult<T>* PyOdeResult::cast() const{
    return reinterpret_cast<const OdeResult<T>*>(this->res);
}
...
py::object PyOdeSolution::_get_frame(const py::object& t) const{
    return py::cast(Array<T>(reinterpret_cast<const OdeSolution<T>*>(this->res)->operator()(t.cast<T>()).data(), this->q0_shape.data(), this->q0_shape.size()));
}
