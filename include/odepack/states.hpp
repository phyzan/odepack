...
public:
    ...
    virtual bool resize_step(T& factor, const T& min_step=0, const T& max_step=inf<T>()){
        //TODO: convert runtime polymorphist to static, since it already uses crtp
...
public:
    ...
    ViewState(const T& t, const vec<T, N>& q_true, const vec<T, N>& q_exposed) : State<T, N>(t, q_true, 0), _q_exposed(q_exposed) {
...
private:
    ...
    vec<T, N> _q_exposed;
    vec<T, N>* _q_exposed_ptr = nullptr;
