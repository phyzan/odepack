...
ODE<T, NDIM>* VectorField<Derived, T, NDIM, AS_VIRTUAL>::get_streamline_ode(const T* x0, T rtol, T atol, T min_step, T max_step, T stepsize, int direction, Integrator method, bool normalized) const{
    if (normalized){
        return new ODE<T, NDIM>(OdeData{.Rhs=[this](T* out, const T& t, const T* q){ THIS->OdeFuncNorm(out, t, q); }}, T{0}, View1D<T, NDIM>{x0, this->ndim()}, rtol, atol, min_step, max_step, stepsize, direction, {}, {}, method);
    } else {
        return new ODE<T, NDIM>(OdeData{.Rhs=[this](T* out, const T& t, const T* q){ THIS->OdeFunc(out, t, q); }}, T{0}, View1D<T, NDIM>{x0, this->ndim()}, rtol, atol, min_step, max_step, stepsize, direction, {}, {}, method);
    }
}
